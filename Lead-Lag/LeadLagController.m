...
Kc = 200; % Compensator gain (to reduce overshoot decrease this but it'll increase the error)
z1 = 25;  % Lead zero
p1 = 70; % Lead pole (to reduce overshoot increase this)
z2 = 10; % Lag zero (to reduce steady-state error increase this)
p2 = 2; % Lag pole (to reduce steady-state error reduce this)
omega = 78; % wn

Gc = Kc * ((s + z1) / (s + p1)) * ((s + z2) / (s + p2)); % Lead-Lag Compensator
G = tf(omega, [1 omega omega^2 0]); % System transfer function
